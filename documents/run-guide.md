...
> [!IMPORTANT]
> **팀원 PC에서 처음 실행하는 경우 → [Section 3. 완전 초기화](#3-완전-초기화-처음부터-다시-올리는-경우)부터 시작하세요.**  
> 이미지가 로컬에 없으면 파드가 `ErrImagePull`로 뜨고 서비스가 되지 않습니다.  
> Section 1은 이미 한 번 세팅된 환경에서 Docker Desktop을 재시작했을 때만 해당됩니다.
