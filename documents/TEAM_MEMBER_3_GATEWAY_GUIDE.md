...
[ ] WebSocket 서버 초기화
    └─ 위치: apps/websocket-service/src/
       ├─ [ ] package.json 작성 (socket.io, express, redis)
       ├─ [ ] index.js 작성 (서버 진입점)
       ├─ [ ] server.js 작성 (Express + Socket.IO 설정)
       ├─ [ ] config.js 작성 (환경변수 로드)
       └─ [ ] 기본 라우터 설정

[ ] Redis 클라이언트 설정
    └─ 위치: apps/websocket-service/src/utils/
       ├─ [ ] redis-client.js 작성 (연결 관리)
       ├─ [ ] constants.js 작성 (key naming)
       └─ [ ] Redis 연결 테스트
...
[ ] API 클라이언트 설정
    └─ 위치: apps/frontend/src/services/
       ├─ [ ] api.js 작성 (axios 인스턴스)
       ├─ [ ] socket.js 작성 (Socket.IO 클라이언트)
       └─ [ ] sse.js 작성 (SSE 클라이언트)

[ ] Docker 설정
    └─ [ ] Dockerfile 작성 (websocket)
    └─ [ ] Dockerfile 작성 (frontend)
    └─ [ ] docker-compose.gateway.yml 작성

[ ] K8s 배포 설정
    └─ 위치: infra/k8s/base/
       ├─ [ ] websocket-service/deployment.yaml
       ├─ [ ] websocket-service/service.yaml
       ├─ [ ] frontend/deployment.yaml
       └─ [ ] frontend/service.yaml

[ ] CI/CD 파이프라인
    └─ 위치: .github/workflows/
       ├─ [ ] ci-websocket.yml 작성 (npm test, eslint)
       └─ [ ] ci-frontend.yml 작성 (vite build, eslint)
...
[ ] QueueStatus 컴포넌트
    └─ 위치: apps/frontend/src/components/QueueStatus/
       ├─ [ ] QueueStatus.jsx (대기 정보 표시)
       ├─ [ ] WaitTimeGauge.jsx (시간 게이지)
...
[ ] WebSocket 기초 준비
    └─ 위치: apps/websocket-service/src/
       ├─ [ ] subscribe_event 이벤트 구현
       ├─ [ ] subscription_confirmed 이벤트 구현
       └─ [ ] connection_info 이벤트 구현
...
[ ] useWebSocket Hook (핵심🔥)
    └─ 위치: apps/frontend/src/hooks/useWebSocket.js
       ├─ [ ] WebSocket 연결 관리
       ├─ [ ] subscribe_event 발송
       ├─ [ ] seat_status_updated 수신 처리
       ├─ [ ] seat_hold_expired 처리
...
[ ] 좌석 상태 전역 상태 관리
    └─ 위치: apps/frontend/src/hooks/useSeatSelection.js
       ├─ [ ] 좌석별 상태 저장 (available, hold, sold)
       ├─ [ ] 사용자 선택 상태 저장
       ├─ [ ] 상태 업데이트 함수
       └─ [ ] 중복 선택 방지
...
[ ] WebSocket 서버 좌석 이벤트
    └─ 위치: apps/websocket-service/src/
       ├─ [ ] seat_status_updated 이벤트 구현
       ├─ [ ] seat_reserved 이벤트 구현
       ├─ [ ] seat_hold_expired 이벤트 구현
       └─ [ ] seat_availability_summary 이벤트 구현 (30초마다)

[ ] Redis Pub/Sub 연동 (인프라)
    └─ 위치: apps/websocket-service/src/services/redis-service.js
       ├─ [ ] Redis Pub/Sub 구독 설정
       ├─ [ ] 이벤트 발행 처리
       └─ [ ] 메시지 브로드캐스트
...
[ ] PaymentForm 컴포넌트
    └─ 위치: apps/frontend/src/components/PaymentForm/
       ├─ [ ] 카드 정보 입력 (PCI DSS 컴플라이언스)
       ├─ [ ] 결제 버튼
       ├─ [ ] 오류 메시지
       └─ [ ] PaymentForm.css (다크 테마)

[ ] ConfirmationPage 컴포넌트
