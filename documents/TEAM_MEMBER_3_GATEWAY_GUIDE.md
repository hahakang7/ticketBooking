...
[x] 토큰 관리
    └─ 위치: apps/frontend/src/services/storage.js
       ...
       └─ [x] 토큰 만료 처리 (JWT exp 디코딩 후 만료 시 자동 삭제)
...
[x] WebSocket 테스트
    └─ 위치: apps/websocket-service/src/test/socket.test.js (vitest, 8개 통과)
       ├─ [x] Socket.IO 클라이언트 연결 테스트 (connection_info 수신, 연결 해제)
       └─ [x] 기본 이벤트 송수신 테스트 (subscribe_event, seat_hold, seat_unhold, seat_reserved)

[x] 단위 테스트
    └─ 위치: apps/frontend/src/test/ (vitest + @testing-library/react, 21개 통과)
       ├─ [x] useQueue Hook 테스트 (useQueue.test.js - 7개)
       ├─ [x] SSE 연결 테스트 (sse.test.js - 7개)
       └─ [x] 토큰 관리 테스트 (storage.test.js - 7개)

[x] 팀원 1에게 정보 제공
    └─ [x] Queue API 응답 형식 확인 (아래 내용 공유 완료)
    │
    │   POST /api/queue/join
    │     Request:  { "user_id": "string", "event_id": "string" }
    │     Response: { "code": 200, "data": { "position": 50, "queue_token": "eyJ...", "total": 500 } }
    │
    │   GET /api/queue/status?user_id=&event_id=
    │     ⚠️ Authorization: Bearer {queue_token} 헤더 필수
    │     Response: { "code": 200, "data": { "position": 45, "total": 500, "is_in_queue": true } }
    │
    │   GET /api/queue/sse?user_id=&event_id=
    │     ⚠️ Authorization: Bearer {queue_token} 헤더 필수
    │     Stream: data: { "status": "waiting"|"ready"|"not_in_queue", "position": int, "total": int }
    │             data: { "status": "ready", "position": 0, "access_token": "eyJ..." }  ← 입장 시
    │
    └─ [x] k6 queue-load-test 상황 공유 (버그 2건 발견 및 전달)
           ├─ 버그 1: /status 호출 시 queue_token Authorization 헤더 누락 → 401 에러 발생
           │          수정 필요: res에서 queue_token 추출 후 헤더에 포함
           └─ 버그 2: teardown의 DELETE /api/queue/leave 엔드포인트 미구현 → 404
                      정리 로직 제거하거나 팀원 2에게 /leave 엔드포인트 추가 요청 필요
```
...
```
[x] SeatSelectionPage 컴포넌트
    └─ 위치: apps/frontend/src/pages/HomePage.jsx (SeatSelectionPage 역할 담당)
       ├─ [x] 좌석 맵 렌더링 (SeatMap.jsx + SectionPanel.jsx)
       ├─ [x] 좌석 클릭 처리 (SeatDetailModal.jsx)
       ├─ [x] 선택 좌석 표시 (SeatDetailModal 선택 요약)
       └─ [x] 가격 계산 (totalPrice = selectedSeats.reduce(...))

[x] SeatMap 컴포넌트 (핵심🔥)
    └─ 위치: apps/frontend/src/components/pages/ (구조는 가이드와 다르나 기능 동일)
       ├─ [x] SeatMap.jsx (야구장 SVG 맵, 등급 클릭 처리)
       ├─ [x] SeatDetailModal.jsx (좌석 행/열 그리드, WebSocket 연동)
       ├─ [x] SectionPanel.jsx (등급/구역 패널)
       └─ [x] styles/components/seat-map.css + seat-detail-modal.css

[x] Seat 상태 표시
    └─ 색상 정의: (seat-detail-modal.css 구현 완료)
       ├─ [x] Available: 선택가능 스타일
       ├─ [x] Hold (임시 점유): 점유중 스타일
       ├─ [x] Sold: 판매완료 스타일
       └─ [x] Selected (사용자 선택): selected 스타일
...
[x] useWebSocket Hook (핵심🔥)
    └─ 위치: apps/frontend/src/hooks/useWebSocket.js
       ...
       ├─ [x] seat_reserved 수신 처리 (sold 상태로 즉시 반영)
...
[x] WebSocket 서버 좌석 이벤트
    └─ 위치: apps/websocket-service/src/
       ...
       ├─ [x] seat_reserved 이벤트 구현 (broadcastBatchSeatUpdate에서 status=sold 시 발행)
...
[x] Redis Pub/Sub 연동 (인프라)
    └─ 위치: apps/websocket-service/src/
       ...
       ├─ [x] 이벤트 발행 처리 (core-api reservation_service._publish_seat_update 구현 완료)
       └─ [x] 메시지 브로드캐스트 (seatService.broadcastBatchSeatUpdate - 배열 형식 처리)

[x] 성능 최적화
    └─ [x] 좌석 렌더링 최적화 (useMemo, useCallback)
       └─ SeatDetailModal: rowMap useMemo, handleSeatClick useCallback, totalPrice useMemo
       └─ SeatMap: getColor/getOpacity/handleClick useCallback
    └─ [x] WebSocket 메시지 배치 처리
       └─ broadcastBatchSeatUpdate로 동일 이벤트 다수 좌석 한 번에 전송

[x] 모바일 최적화
    └─ [x] 터치 이벤트 처리 (onTouchEnd + touch-action: manipulation으로 더블탭 줌 방지)
    └─ [x] 화면 크기별 좌석 맵 반응형 레이아웃
       └─ seat-detail-modal.css: 480px/360px 브레이크포인트 + 좌석 크기 조정
       └─ Modal.css: 모바일에서 바텀시트 형태 (border-radius 16px 상단)
    └─ [ ] 모바일 성능 테스트 (Lighthouse - 실행 환경 필요, 수동 측정)

[x] 고급 테스트
    └─ 위치: apps/websocket-service/src/test/socket.test.js (14개 통과)
       ├─ [x] WebSocket 연결 테스트
       ├─ [x] 메시지 전송/수신 테스트
       ├─ [x] 0.1초 이내 업데이트 성능 테스트 (broadcastBatchSeatUpdate, seat_hold 각각 검증)
       ├─ [x] 재연결 테스트 (재구독 후 브로드캐스트 수신 확인)
       └─ [x] 동시 100+ 연결 테스트 (100개 동시 연결 + 브로드캐스트 전체 수신)
...
[x] PaymentForm 컴포넌트
    └─ 위치: apps/frontend/src/components/PaymentForm/
       ...
       └─ [x] 실제 결제 API 연동 (POST /v1/reservations → POST /v1/payments 순서 호출)
