...
팀원 3 K8s Service(`websocket-service/service.yaml`)에는 `sessionAffinity: ClientIP`가 있지만 Ingress 레벨이 없으면 Pod 재배포 시 연결 끊김.
...
```
ws.example.com 호스트 규칙에만 적용 (api, frontend에는 불필요).

**검증:** `kubectl apply --dry-run=client -f infra/k8s/base/shared/ingress.yaml`
...
**파일:** `apps/frontend/src/components/pages/SeatMap.jsx` (또는 실제 경로 확인 후)

현재 상태: 구역(섹션) 클릭만 지원, 개별 좌석의 available/hold/sold 색상 없음.  
`useWebSocket.js`는 `seat_status_updated` 이벤트를 수신하고 있으므로 훅은 준비됨.

추가 구현:
1. SeatMap이 `useWebSocket`에서 좌석 상태 Map을 받아서
2. 각 좌석 SVG 요소에 상태별 색상 적용 (`available: 초록`, `hold: 노랑`, `sold: 빨강`)
3. 내가 선택한 좌석 구분 표시 (`mine: 파랑`)

**검증:** WebSocket 이벤트 mock 후 색상 변경 확인

---

#### Step 2-9. 대기열 → 좌석 선택 → 결제 전체 플로우 연결 검증
**파일:** `apps/frontend/src/` App.jsx, pages/

현재 `SeatSelectionPage.jsx`가 없고 `pages/SeatMap.jsx`가 독립적으로 존재.  
전체 여정(QueuePage → SeatMap → PaymentPage)이 라우팅으로 연결되어 있는지 확인.

확인 및 추가 구현:
- App.jsx 라우팅 확인 (`/queue` → `/seats` → `/payment` 경로)
- queue_token 받은 후 자동으로 좌석 선택 페이지로 이동하는 흐름
- PaymentPage에서 좌석 hold → 결제(mock) → 완료 페이지 이동

---

#### Step 2-10. 프론트엔드 번들 최적화
**파일:** `apps/frontend/vite.config.js` (또는 .ts)

현재 번들 크기 목표: < 150KB (가이드 기준).  
적용 방법:
- `vite.config`에 `rollupOptions.output.manualChunks` 설정으로 페이지별 코드 분할
- 사용하지 않는 Tailwind 클래스 제거 (`content` 경로 정확히 설정)
- `vite-bundle-analyzer` 또는 `rollup-plugin-visualizer`로 번들 시각화 후 큰 의존성 파악

**검증:** `npm run build` 후 `dist/assets/` 파일 크기 확인
...
| 항목 | 담당 | 이유 |
|------|------|------|
| Ingress sticky session 어노테이션 | 팀원 1+3 | K8s Service만으론 부족, Ingress 레벨 필수 |
| GZipMiddleware 추가 (`main.py`) | 팀원 2 | API 응답 크기 최소화, P95 달성 필수 도구 |
| slow request 임계값 500ms→300ms | 팀원 2 | KPI와 logger 기준 일치 필요 |
| 전체 여정 라우팅 연결 검증 | 팀원 3 | SeatSelectionPage 미존재로 흐름 끊길 수 있음 |
| 대기열→좌석 자동 이동 처리 | 팀원 3 | queue_token 발급 후 페이지 전환 미구현 가능성 |
| Grafana 대시보드 JSON 생성 | 팀원 1 | 발표 시 실시간 메트릭 시각화 필요 |
| Docker Compose E2E 통합 실행 | 팀원 1 주도 | 개별 서비스는 됐지만 통합 기동 미검증 |
| `infra/k8s/base/core-api/` namespace 불일치 수정 | 팀원 1+2 | websocket/frontend는 `namespace: ticket-system` 있지만 core-api는 없음 |
...
| **Day 3~5** | Phase 2: 팀원별 병렬 개발 (총 10개 Step) | 각자 독립 |
